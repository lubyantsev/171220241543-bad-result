...
def calculate_standard_deviation(stock_data):
    """
    Рассчитывает стандартное отклонение цен закрытия.

    :param stock_data: DataFrame с данными о записях акций.
    :return: Стандартное отклонение цен закрытия.
    """
    if 'Close' in stock_data.columns:
        std_dev = np.std(stock_data['Close'])
        return std_dev
    else:
        raise ValueError("Данные о ценах закрытия отсутствуют в DataFrame.")